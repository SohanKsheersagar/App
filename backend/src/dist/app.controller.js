"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AppController = void 0;
const common_1 = require("@nestjs/common");
const wedding_service_1 = require("./services/wedding.service");
let AppController = class AppController {
    constructor(weddingService) {
        this.weddingService = weddingService;
    }
    async createWedding(groomName, brideName, date) {
        return this.weddingService.createWedding(groomName, brideName, date);
    }
    async getAllWeddings() {
        return this.weddingService.findAllWeddings();
    }
};
exports.AppController = AppController;
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)('groomName')),
    __param(1, (0, common_1.Body)('brideName')),
    __param(2, (0, common_1.Body)('date')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, Date]),
    __metadata("design:returntype", Promise)
], AppController.prototype, "createWedding", null);
__decorate([
    (0, common_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], AppController.prototype, "getAllWeddings", null);
exports.AppController = AppController = __decorate([
    (0, common_1.Controller)('weddings'),
    __metadata("design:paramtypes", [wedding_service_1.WeddingService])
], AppController);
//# sourceMappingURL=app.controller.js.map